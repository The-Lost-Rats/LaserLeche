...
public class ParallaxController : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform grassObjectsParent;
    ...
    [SerializeField]
    private List<GameObject> backgroundObjPrefabs;
    ...
    protected void Start()
    {
        ...
        {
            GameObject grassObj = Instantiate(
                grassPrefab,
                new Vector2(GetInitXPos(i, 0), 0),
                Quaternion.identity,
                grassObjectsParent
            );
        ...
        foreach (
            ParallaxObject existingBackgroundObj in backgroundObjectsParent.GetComponentsInChildren<ParallaxObject>()
        )
        {
        ...
        foreach (GameObject backgroundPrefab in backgroundObjPrefabs)
        {
            ...
            backgroundObj.transform.position = new Vector2(
                GetInitXPos(parallaxObj.mapCellLocation, parallaxObj.parallaxDistance),
                backgroundObj.transform.position.y
            );
    ...
    protected void FixedUpdate()
    {
        ...
        if (playerPosX > Constants.PIXELS_PER_UNIT * (mapSize / 2))
            playerPosX -= Constants.PIXELS_PER_UNIT * mapSize;
        if (playerPosX < -Constants.PIXELS_PER_UNIT * (mapSize / 2))
            playerPosX += Constants.PIXELS_PER_UNIT * mapSize;
    ...
    {
        if (cellLoc >= (mapSize / 2))
            cellLoc -= mapSize;
        float startingRawXPos =
            (Constants.PIXELS_PER_UNIT * cellLoc) + (Constants.PIXELS_PER_UNIT / 2) - playerPosX;
    ...
    {
        int cell = Mathf.RoundToInt(
            (
                ((posX * Mathf.Pow(2, parallaxDist)) + playerPosX - (Constants.PIXELS_PER_UNIT / 2))
    ...
    {
        float pos =
            ((mapCell * mapCellSize) / Constants.PIXELS_PER_UNIT)
            - playerPosX
            + (Constants.PIXELS_PER_UNIT / 2);
        while (pos > Constants.PIXELS_PER_UNIT * (mapSize / 2))
            pos -= Constants.PIXELS_PER_UNIT * mapSize;
        while (pos < -Constants.PIXELS_PER_UNIT * (mapSize / 2))
            pos += Constants.PIXELS_PER_UNIT * mapSize;
    ...
    public int GetScreenBoundsForDistance(int parallaxDist)
    {
        return Mathf.CeilToInt(
            (Constants.PIXELS_PER_UNIT * (mapSize / 2)) / Mathf.Pow(2, parallaxDist)
        );
    }
